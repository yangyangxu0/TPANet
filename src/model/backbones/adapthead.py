...
#                                           for t in self.tasks})
#
#        self.specific_tasks = nn.ModuleList([nn.Conv2d(self.in_channels, out_dim, kernel_size=1, groups=8)  for t in range(len(self.tasks))])
...
#
#        self.cross_atts = nn.ModuleList([nn.MultiheadAttention(out_dim, num_heads=4) for t in range(len(self.tasks))])
#        self.init_weights()
#
#
#    def forward(self, inp, inp_shape, **kwargs):
#        out=[]
#        # fpn_blocks 4 layers output diff scale tensor, out:4 same scale tensor
#        #for blk in self.fpn_blocks:
#        #    out_fpn = blk(inp)
#        #    out.append(out_fpn)
